...
public class Runner2WellFound implements Runnable {
	...
	public Runner2WellFound(String starturl, String email, String password, ArrayList<String> btn,
...
//		WebDriver  driver = new FirefoxDriver();
		...
		while (driver.findElements(By.xpath(
			...
			try {
				...
				String tempLastItemId = driver.findElements(By.className("JobCard_trackingLink__zUSOo"))
						.get(driver.findElements(By.className("JobCard_trackingLink__zUSOo")).size() - 1)
						.getAttribute("href");
		...
		loop: for (int jobInd = startJobPage; jobInd < jobPages.size(); jobInd++) {
			try {
				while(!jobPages.get(jobInd).isDisplayed() || !jobPages.get(jobInd).isEnabled()) {
					Thread.sleep(1000);
