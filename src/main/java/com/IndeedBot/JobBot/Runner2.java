...
public class Runner2 implements Runnable{
	...
	public Runner2(String starturl, String email, String password, ArrayList<String> btn,
	...
	public void run() {
		...
		obj.addArguments("user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36");
...
//		driver.close();
		...
		// driver.navigate().to("https://www.indeed.com/?from=gnav-jobseeker-profile--profile-one-frontend");
		driver.navigate().to(starturl);
		while (true) {
			loop: for (WebElement jobPage : driver.findElements(By.className("slider_container"))) {
				try {
					jobPage.click();
					Thread.sleep(7000);
					if (driver.findElements(By.className("ia-IndeedApplyButton")).size() > 0
							&& driver.findElement(By.className("ia-IndeedApplyButton")).getText().equals("Apply now")) {
						driver.findElement(By.className("ia-IndeedApplyButton")).click();
					} else {
					...
					for (int i = 0; i < 10; i++) {
						if (driver.findElements(By.xpath(
								"//label[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'verification code')]"))
								.size()
								+ driver.findElements(By.xpath(
										"//label[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'password')]"))
										.size() == 0) {
							fillInInfo(driver);
...
//					e.printStackTrace();
	...
	public void fillInInfo(WebDriver driver) throws InterruptedException {
		try {
			...
			try {
				for (WebElement label : driver.findElements(By.cssSelector(".css-132vple.es2vvo70"))) {
...
//		driver.findElement(By.xpath("//input[@type='email']")).sendKeys(email);
		this.sendKeysSlowly(driver.findElement(By.xpath("//input[@type='email']")), email);
		Thread.sleep(2000);
		while (true) {
			try {
				if (driver.findElements(By.xpath(
						"//button//span[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'continue')]"))
						.size() > 3)
					driver.findElements(By.xpath(
							"//button//span[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'continue')]"))
							.get(3).click();
...
//				driver.switchTo().activeElement().sendKeys(password);
				this.sendKeysSlowly(driver.switchTo().activeElement(), password);
				Thread.sleep(2000);
				break;
			} catch (Exception e) {
				Thread.sleep(1000);
